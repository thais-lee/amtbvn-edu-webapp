...
import { Outlet, createRoute, Link } from '@tanstack/react-router';
import { Layout, Button, Spin } from 'antd';
import { MenuOutlined, HomeOutlined } from '@ant-design/icons';
import { DesktopDrawerNav } from '@/shared/components/layouts/app/desktop-drawer-nav'; // Import drawer nav
  ...
  return (
    <Layout style={{ minHeight: '100vh' }}>
      <Header style={{ display: 'flex', alignItems: 'center', background: '#fff', padding: '0 24px', borderBottom: '1px solid #f0f0f0' }}>
         <Button type="text" icon={<MenuOutlined />} onClick={showDrawer} style={{ marginRight: '16px' }}/>
         <Link to="/d/home" style={{ color: 'inherit', textDecoration: 'none', display: 'flex', alignItems: 'center' }}>
              <HomeOutlined style={{ marginRight: '8px', fontSize: '20px' }}/>
              <span style={{ fontWeight: 'bold', fontSize: '18px' }}>OLS (Desktop)</span>
         </Link>
         {/* Thêm User menu, etc. */}
