...
export const Route = createFileRoute('/_app/d/library/lectures/')({
...
function RouteComponent() {
  ...
  // Fetch parent categories (parentId = 18 for lectures)
  const parentCategoryQuery = useQuery({
    queryKey: ['categories-lectures'],
    queryFn: () => categoryService.getAllCategories({ parentId: 18 }),
  });

  // Fetch child categories for the selected parent
  const childCategoryQuery = useQuery({
    queryKey: ['categories-lectures', activeTab],
    enabled: !!activeTab,
  ...
  const { data: libraryMaterials, isLoading: isMaterialsLoading } = useQuery({
    queryKey: ['library-materials-lectures', activeChildTab],
    queryFn: () =>
  ...
  return (
    <div className="library-screen">
      ...
      <PageHeader
        title={t('Lectures')}
        subtitle="Kho bài giảng, video, audio Phật pháp"
      />
